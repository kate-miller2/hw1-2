...
2.  Clone your Github repository on your local machine
    ...
    ``` bash
    $ git clone <<https://github.com/kate-miller2/hw1.git>>
    $ cd hw-1
    ```
...
3.  In order to activate the `R` environment for the homework, make sure
    ...
    Follow the instructions in order to make sure that `renv` is
...
# Based on the output, the vector contains character data types.
...
# 4 elements of 'my_vec_double' are greater than zero, as displayed by the output FALSE.
...
sorted_double <- sort(my_vec_double)
sorted_double
...
matrix1 <- matrix(NA, nrow = 3, ncol = 3)
...
matrix2 <- matrix(NA, nrow = 2, ncol = 100)
...
I expect the col_wise_scan to take shorter to run because in R, items in columns are stored next to each other in memory, whereas with rows, the items are not stored next to each other in memory. 
...
The row_wise_time took longer to run, at 0.0001068115 seconds.
...
The rows took longer to calculate than the columns did in each instance. Therefore, rows will usually take longer than columns. Another important conclusion to make is that as the size of the matrix increased throughout the different trials, the time for both rows columns increased, though columns were always still faster than rows even at the increased times.
